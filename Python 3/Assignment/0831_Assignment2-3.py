...
while True:
    ...
    if m_choice == 0: #주문 목록 확인 및 결제(영수증)
        ...
        print()
        print("결제창으로 이동하시겠습니까? \n[Y]es \n[N]o")
        ynchoice = str(input()) #결제 수단 선택
        if ynchoice == 'Y':
            print("결제 수단을 선택해주세요.\n현금 : [C]ash\n카드 : Car[D]")
        elif ynchoice == 'N':
            print("취소를 선택하였습니다. 처음 화면으로 돌아갑니다.") 
            continue
        else:
            print("잘못 입력하셨습니다. 처음 화면으로 돌아갑니다.")
            continue
        while True: #결제 과정
            ynchoice = str(input())
            if ynchoice == "C":
                ...
                if cash == sum(total_price):
                    ...
                    for k in range(len(cart.keys())): #영수증
                        print(f'{"%s" %"".join(last_order[k][0]):<10}',f'{"%s원" %"".join(map(str,last_order[k][1])):>10}',"%d개" %last_quantity[k])
                    print("총 금액 : %d원" %sum(total_price))
                    #장바구니 리셋
